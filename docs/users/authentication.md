...
This document summarizes the current authentication experience implemented in `web` and `api`.

## Overview

Vortex now supports a full primitive auth flow with dedicated pages:
...
Vortex uses **email as the user identity** for authentication requests:
...
- Auth pages include the same two-column Vortex wordmark treatment as the main app shell.
