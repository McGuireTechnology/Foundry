        ...
        HStack(alignment: .center, spacing: isLarge ? 14 : 12) {
            ...
            VStack(alignment: .leading, spacing: isLarge ? 6 : 2) {
                ...
                Text("by McGuire Technology, LLC")
                    ...
                    .foregroundStyle(VortexPalette.textSecondary)
                    .lineLimit(1)
...
private struct BannerView: View {
    ...
    var body: some View {
        Text(text)
            ...
            .foregroundStyle(VortexPalette.bannerText)
            .frame(maxWidth: .infinity, alignment: .leading)
        .padding(.horizontal, 14)
        .padding(.vertical, 12)
        .background(
            RoundedRectangle(cornerRadius: 10, style: .continuous)
                .fill(VortexPalette.bannerBackground)
        )
        .overlay(
            RoundedRectangle(cornerRadius: 10, style: .continuous)
                .stroke(VortexPalette.bannerBorder, lineWidth: 1)
        )
    }
}

struct VortexLogoMark: View {
    var body: some View {
        GeometryReader { geometry in
            ...
            ZStack {
                ...
                .fill(VortexPalette.logoMagenta)
                ...
                .fill(VortexPalette.logoYellow)

                Path { path in
                    path.move(to: CGPoint(x: width * 0.5, y: height * 0.535))
                    path.addLine(to: CGPoint(x: width * 0.5, y: height))
                }
                .stroke(VortexPalette.logoCenterLine, style: StrokeStyle(lineWidth: 2.2, lineCap: .round))
            }
        }
...
struct VortexPanelStyle: ViewModifier {
    func body(content: Content) -> some View {
        content
            .background(
                RoundedRectangle(cornerRadius: 20, style: .continuous)
                    .fill(VortexPalette.panelBackground)
            )
            .overlay(
                RoundedRectangle(cornerRadius: 20, style: .continuous)
                    .stroke(VortexPalette.panelBorder, lineWidth: 1)
            )
    }
}

extension View {
    func vortexPanelStyle() -> some View {
        modifier(VortexPanelStyle())
    }
}
