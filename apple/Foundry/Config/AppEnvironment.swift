...
enum VortexRuntimeEnvironment: String, CaseIterable {
    case local
    case staging
    case production

    static let defaultValue: VortexRuntimeEnvironment = .local

    static func resolve(from rawValue: String?) -> VortexRuntimeEnvironment {
...
struct AppEnvironment {
    ...
    let runtimeEnvironment: VortexRuntimeEnvironment
    let baseURL: URL
    ...
    private static func makeCurrent() -> AppEnvironment {
        let runtimeEnvironment = VortexRuntimeEnvironment.resolve(from: processOrInfoValue(for: "FOUNDRY_APP_ENV"))
